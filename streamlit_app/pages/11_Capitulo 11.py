...
st.set_page_config(
    page_title="Capítulo 11 - Redes Neuronales",
    page_icon="🧠",
    layout="wide",
)
...
# ======== TÍTULO ========
st.markdown("<h1 class='main-title'>🧠 Redes Neuronales</h1>", unsafe_allow_html=True)
st.markdown("<p class='subtitle'>Entrena, clasifica y evalúa dígitos usando inteligencia artificial</p>", unsafe_allow_html=True)

# ======== ARCHIVOS ========
files = {
    "./chapter11/prueba1.py": {"name": "Entrenador de Dígitos", "icon": "🏋️"},
    "./chapter11/prueba2.py": {"name": "Clasificador de Dígitos", "icon": "🔢"},
    "./chapter11/prueba3.py": {"name": "Evaluador de Precisión", "icon": "📊"},
}
        ...
        if idx < len(script_items):
            ...
            with col:
                ...
                with col1:
                    ...
                    if running:
                        st.button("▶️ Iniciar", key=f"start_{label}", disabled=True)
                    else:
                        st.button(
                            "▶️ Iniciar",
                            key=f"start_{label}",
                ...
                with col2:
                    ...
                    if not running:
                        st.button("⛔ Detener", key=f"stop_{label}", disabled=True)
                    else:
                        st.button(
                            "⛔ Detener",
                            key=f"stop_{label}",
...
st.markdown("""
    <div class='footer'>
        <strong>Capítulo 11</strong> — Redes Neuronales con OpenCV y Python<br>
        Entrenamiento y clasificación de dígitos escritos a mano
    </div>
""", unsafe_allow_html=True)
