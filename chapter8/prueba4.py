...
def add_tracking_paths(frame, tracking_paths):
    ...
    if feature_points is not None: 
        for x, y in np.float32(feature_points).reshape(-1, 2): 
            tracking_paths.append([(int(x), int(y))])  # Asegurarse de que sean enteros
...
def start_tracking(cap, scaling_factor, num_frames_to_track, num_frames_jump, tracking_params): 
    ...
    while True: 
        ...
        if not ret:
            break  # Asegúrate de que se lea correctamente el frame
        ...
        if len(tracking_paths) > 0: 
            ...
            for tp, (x, y), good_points_flag in zip(tracking_paths, feature_points, good_points): 
                ...
                tp.append((int(x), int(y)))  # Asegurarse de que sean enteros

                if len(tp) > num_frames_to_track: del tp[0] 
                new_tracking_paths.append(tp) 

                cv2.circle(output_img, (int(x), int(y)), 3, (0, 255, 0), -1)  # Asegurarse de que sean enteros
...
if __name__ == '__main__': 
    cap = cv2.VideoCapture(0)  # Cambia esto a un archivo de video si lo deseas
