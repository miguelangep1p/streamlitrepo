...
def compute_energy_matrix_modified(img, rect_roi): 
    ...
    # Set energy values in the ROI to 0 
...
def add_vertical_seam(img, seam, num_iter): 
    ...
    for row in range(rows): 
        ...
        # Insert a value between two columns by averaging neighbors 
        for i in range(3): 
...
def remove_object(img, rect_roi): 
    ...
    # Fill the region with surrounding values 
    for i in range(num_seams): 
        seam = find_vertical_seam(img, energy) 
        img = remove_vertical_seam(img, seam) 
    ...
    cv2.imshow('Input', cv2.resize(img_input , (400,520))) 
    cv2.imshow('Output', img_output) 
    cv2.waitKey() 

if __name__ == '__main__': 
    # Cargar una imagen por defecto
    img_input = cv2.imread('./images/persona.jpg')  # Asegúrate de tener esta imagen en el mismo directorio
    drawing = False 
    ...
    while True:
        ...
        c = cv2.waitKey(10) 
        if c == 27:  # Presiona Esc para salir
            break 
