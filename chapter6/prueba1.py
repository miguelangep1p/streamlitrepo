...
# Redimensionar imagen solo para visualización
def resize_for_display(img, width=500):
    h, w = img.shape[:2]
    if w > width:
        ratio = width / float(w)
...
if __name__=='__main__': 
    ...
    if len(sys.argv) > 1:
        img_input = cv2.imread(sys.argv[1])  # Imagen pasada por la línea de comandos
    else:
        img_input = cv2.imread('./images/pelota.jpg')  # Imagen por defecto si no se pasa argumento
    
    if len(sys.argv) > 2:
        num_seams = int(sys.argv[2])  # Número de seams pasado por la línea de comandos
    else:
        num_seams = 5  # Número de seams por defecto
    ...
    # Realizar el procesamiento a resolución completa
    ...
    img_input_display = resize_for_display(img_input)
    img_overlay_display = resize_for_display(img_overlay_seam)
    img_output_display = resize_for_display(img_output)
    
    # Mostrar imágenes redimensionadas
    cv2.imshow('Input', img_input_display) 
    cv2.imshow('Seams', img_overlay_display)
    cv2.imshow('Output', img_output_display) 
    cv2.waitKey() 
